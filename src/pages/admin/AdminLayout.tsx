...
const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    { label: 'Overview', path: '/admin', icon: <Home className="w-5 h-5" aria-hidden="true" /> },
    { label: 'Doctors Approval', path: '/admin/doctors', icon: <Stethoscope className="w-5 h-5" aria-hidden="true" /> },
    { label: 'Articles & Content', path: '/admin/posts', icon: <FileText className="w-5 h-5" aria-hidden="true" /> },
    { label: 'Events & Seminars', path: '/admin/events', icon: <Calendar className="w-5 h-5" aria-hidden="true" /> },
    { label: 'User Accounts', path: '/admin/users', icon: <Users className="w-5 h-5" aria-hidden="true" /> },
  ];

  return (
    <div className="min-h-screen bg-ayush-ivory flex flex-col md:flex-row">
      {/* Sidebar */}
      <aside className="w-full md:w-64 bg-ayush-forest text-ayush-ivory flex-shrink-0" aria-label="Admin navigation">
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          ...
          <Link to="/" className="text-xs font-ui text-ayush-gold hover:underline flex items-center" aria-label="View main website">
            View Site
          </Link>
        </div>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto" id="admin-main-content">
        <Outlet />
      </main>
    </div>
  );
