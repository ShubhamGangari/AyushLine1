...
const ArticleDetail = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Start fetching post & approved posts in parallel
      ...
      setPost(article);
      if (article && allPosts) {
        const sameSystem = allPosts.filter(
          (p) => String(p.id) !== String(id) && (p.system || '').toLowerCase() === (article.system || '').toLowerCase()
        );
        setRelated((sameSystem.length >= 2 ? sameSystem : allPosts.filter((p) => String(p.id) !== String(id))).slice(0, 3));
      }
      setLoading(false);
    };
