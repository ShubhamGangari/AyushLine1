...
export const Dashboard = ({ defaultTab }: DashboardProps) => {
  ...
  useEffect(() => {
    if (user?.id) {
      void getProfile(user.id).then((p) => {
        ...
        const role = profileRole || authRole || 'user';
        setActiveRole(role);
        setLoading(false);
      });
    } else if (user) {
      setActiveRole((user.role as UserRole) || 'user');
      setLoading(false);
    } else {
      setLoading(false);
    }
  ...
  const loadUserData = async () => {
    if (activeRole === 'doctor' || user?.role === 'doctor') {
      const docAppts = await getAppointmentsForDoctor(user?.id || '1', user?.name || userEmail);
  ...
  const handleDoctorApptAction = async (id: string | number, newStatus: 'confirmed' | 'cancelled') => {
    await updateAppointmentStatus(id, newStatus);
    setActionMessage(newStatus === 'confirmed' ? 'Appointment accepted & approved! Patient notified.' : 'Appointment request declined.');
    await loadUserData();
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          <div className="w-14 h-14 rounded-2xl bg-ayush-sage text-ayush-forest font-bold text-xl flex items-center justify-center flex-shrink-0">
                            {activeRole === 'doctor' || user?.role === 'doctor' ? '👤' : '👨‍⚕️'}
                          </div>
                          <div>
                            <h3 className="font-display font-bold text-xl text-ayush-forest">
                              {activeRole === 'doctor' || user?.role === 'doctor'
                                ? appt.patient_name || 'Patient Applicant'
                                : `Dr. ${appt.doctor_name || 'AYUSH Practitioner'}`}
                            </h3>
                            <p className="text-sm text-ayush-gold font-bold font-ui">
                              {activeRole === 'doctor' || user?.role === 'doctor'
                                ? `Patient Email: ${appt.patient_email || 'Provided on booking'}`
                        ...
                        {activeRole === 'doctor' || user?.role === 'doctor' ? (
                          <div className="flex items-center justify-end gap-2 w-full">
                            {isPending && (
                              <>
                                <button
                                  onClick={() => handleDoctorApptAction(appt.id, 'confirmed')}
                                  className="flex-1 px-4 py-2.5 bg-emerald-600 text-white rounded-xl font-bold text-sm hover:bg-emerald-700 transition-all flex items-center justify-center gap-1.5 shadow-sm"
                                >
                                  <CheckCircle2 className="w-4 h-4" /> Accept & Approve
                                </button>
                                <button
                                  onClick={() => handleDoctorApptAction(appt.id, 'cancelled')}
                                  className="px-4 py-2.5 bg-rose-100 text-rose-700 rounded-xl font-bold text-sm hover:bg-rose-200 transition-all flex items-center gap-1"
                                >
                            ...
                            {isConfirmed && (
                              <span className="px-4 py-2 bg-emerald-100 text-emerald-800 rounded-xl font-bold text-sm flex items-center gap-1.5">
                                <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Approved & Confirmed
                              </span>
                            )}
                            {isCancelled && (
                              <span className="px-4 py-2 bg-rose-100 text-rose-700 rounded-xl font-bold text-sm flex items-center gap-1.5">
                                <XCircle className="w-4 h-4 text-rose-600" /> Request Declined
                              </span>
                            )}
                          </div>
                        ) : (
