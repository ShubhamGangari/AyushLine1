  ...
  useEffect(() => {
   ...
   const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
     ...
     try {
       ...
       setSuccessMsg('Photo selected and compressed! Click "Save Profile" to update permanently.');
       setTimeout(() => setSuccessMsg(''), 4000);
     } catch (err: any) {
       setErrorMsg(err.message || 'Failed to process image. Max 2MB limit.');
     }
   };
          ...
          {activeTab === 'profile' && (
            <form onSubmit={handleSaveProfile} className="space-y-6">
              ...
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-ayush-charcoal/10">
                ...
                <div className="mt-5 p-4 rounded-2xl bg-ayush-ivory/60 border border-ayush-forest/15 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                    <span className="block text-xs font-ui font-bold text-ayush-charcoal/60 uppercase tracking-wider">Verified Account Category</span>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="px-3 py-1 rounded-full bg-ayush-forest text-white font-ui font-bold text-xs uppercase tracking-wider flex items-center gap-1.5">
                        {role === 'doctor' && '🩺 Certified AYUSH Practitioner / Doctor'}
                        {role === 'org' && '🏥 AYUSH Recognized Institution / Org'}
                        {(role === 'student' || role === 'user') && '🎓 AYUSH Student / Scholar / Patient'}
                  ...
                  {role !== 'doctor' && (
                    <Link
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-ayush-gold/20 text-ayush-forest font-ui font-bold text-xs hover:bg-ayush-gold transition-all w-fit"
                    >
